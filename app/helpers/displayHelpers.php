...
\n";
	...
	jQuery(document).ready(function() {
		...
		if($t_item->getPrimaryKey()) {
			$digitoolUrls = getDigitoolUrls($t_item->get('digitoolUrl')); //LIBIS
			if (sizeof($va_reps) > 0 || sizeof($digitoolUrls) > 0) {      //LIBIS	
				$va_imgs = array();
			...
			{				
				...
				//libis_start
				if (sizeof($digitoolUrls) > 0) {
                    foreach ($digitoolUrls as $digitoolUrl) {
                            //width:'100%',
                        $digitoolThumbnail = getDigitoolThumbnailUrl($digitoolUrl);
		...
		";
					if (sizeof($va_reps) > 1 || sizeof($digitoolUrls) > 1) {	//LIBIS
						$vs_buf .= "
