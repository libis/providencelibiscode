...
<?php
		...
		if(is_array($this->getVar("sorts")) && (sizeof($this->getVar("sorts")) > 0)){
            // libis_start
//            $temp1 = array_merge(array("ca_objects.object_id"=>"chronologie"),$this->getVar("sorts"));
             $temp1 = $this->getVar("sorts");
            foreach($temp1 as $vs_sort => $vs_option){
                //libis if toegevoegd
//                if(stripos($vs_sort,'ca_objects.object_id') !== false || stripos($vs_option,'objectnaam')!==false|| stripos($vs_option,'inventarisnummer')!==false|| stripos($vs_option,'titel')!==false || stripos($vs_option,'standplaats')!==false || stripos($vs_option,'Materiaal Term')!==false ||  stripos($vs_option,'trefwoorden objectcategorie')!==false || stripos($vs_sort,'ca_objects.objectStijlType') !== false || stripos($vs_sort,'objectTechniekType')!==false || stripos($vs_sort,'ca_entities') !==false || stripos($vs_sort,'ca_occurrences') !==false || stripos($vs_sort,'ca_loans') !==false ||stripos($vs_sort,'ca_collections') !==false) {
